...
router.get('/campaigns/:auditId', requireAuth, (req, res) => {
  try {
    const db = getDb();
    const audit = db.prepare('SELECT * FROM audits WHERE id = ?').get(req.params.auditId);
  ...
  try {
    ...
    const transaction = db.transaction(() => {
      for (const item of items) {
        const campaign = db.prepare('SELECT * FROM campaigns WHERE id = ? AND audit_id = ?').get(item.campaign_id, audit_id);
    ...
    res.status(500).json({ error: 'Erro ao salvar criativos: ' + err.message });
  }
});
...
router.get('/all', requireAuth, requireRole('LIDERANCA'), (req, res) => {
  try {
    const db = getDb();
...
function generateCreativeAnalysis(campaign, copyText, videoLink, productPrice) {
  const points = [];
  const replication = [];
...
function generateOverallStrengths(creatives) {
