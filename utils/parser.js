...
const FULL_MAP = { ...COLUMN_MAP, ...EN_ALIASES };
...
function parseFile(filePath) {
  ...
  for (const row of mapped) {
    ...
    c._ctr_sum = (c._ctr_sum || 0) + parseNumber(row.ctr_link);
    c._cpc_sum = (c._cpc_sum || 0) + parseNumber(row.cpc);
    c._lp_rate_sum = (c._lp_rate_sum || 0) + parseNumber(row.lp_rate);
    c._hook_sum = (c._hook_sum || 0) + parseNumber(row.hook_rate);
    c._count++;
  }
